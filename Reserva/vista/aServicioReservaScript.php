<?php
	...
	$consulta="SELECT r.idServicio, s.nombre FROM servicio s INNER JOIN reserva r ON (s.idServicio = r.idServicio) WHERE nombre='".$id."' AND ci='".$_SESSION["txCedula"]."';";
	...
	while($co = mysql_fetch_array($consultaScript, MYSQL_ASSOC))
	{
		if($valorDD == "Hoy")
		{
			...
			$autorizar="UPDATE prestamo SET fechaReserva='".$annioActual2."-".$mesActual2."-".$diaActual2."', fechaAtencion='".$annioActual2."-".$mesActual2."-".$diaActual2."', estado='Autorizado' WHERE idServicio= '".$co["idServicio"]."' AND ci='".$_SESSION["txCedula"]."';";
		...
		else
		{
			...
			if($diaSiguiente > 30 && $mesActual== 4)
			{
				...
				$autorizar="UPDATE reserva SET fechaReserva='".$annioActual2."-".$mesActual2."-".$diaActual2."', fechaAtencion='".$annioSiguiente."-".$mesSiguiente."-".$diaSiguiente."', estado='Autorizado para Mañana' WHERE idServicio= '".$co["idServicio"]."' AND ci='".$_SESSION["txCedula"]."';";
				$autorizarScript=mysql_query($autorizar, $conexion);
				while($rl=mysql_fetch_array($restarServicioScript, MYSQL_ASSOC))
				{
					$sub=$rl["stockServicio"]-1;
			...
			{
				$autorizar="UPDATE reserva SET fechaReserva='".$annioActual2."-".$mesActual2."-".$diaActual2."', fechaAtencion='".$annioActual."-".$mesActual."-".$diaSiguiente."', estado='Autorizado para Mañana' WHERE idServicio= '".$co["idServicio"]."' AND ci='".$_SESSION["txCedula"]."';";
				$autorizarScript=mysql_query($autorizar, $conexion);
				while($rl=mysql_fetch_array($restarServicioScript, MYSQL_ASSOC))
				{
					$sub=$rl["stockServicio"]-1;
			...
			if($diaSiguiente > 30 && $mesActual==6)
			{
				...
				$mesSiguiente = $mesActual + 1;
				$annioSiguiente=$annioActual;
				$autorizar="UPDATE reserva SET fechaReserva='".$annioActual2."-".$mesActual2."-".$diaActual2."', fechaAtencion='".$annioSiguiente."-".$mesSiguiente."-".$diaSiguiente."', estado='Autorizado para Mañana' WHERE idServicio= '".$co["idServicio"]."' AND ".$_SESSION["txCedula"]."';";
				$autorizarScript=mysql_query($autorizar, $conexion);
				while($rl=mysql_fetch_array($restarServicioScript, MYSQL_ASSOC))
				{
					$sub=$rl["stockServicio"]-1;
